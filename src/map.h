...
namespace oslam
{
    ...
    class GlobalMap
    {
       ...
       public:
        ...
        void add_object(const Frame &r_frame, const InstanceImage &r_instance_image, const Eigen::Matrix4d &r_camera_pose);

        void integrate_background(const Frame &r_frame, const Eigen::Matrix4d &r_camera_pose);
        void integrate_object(const Frame &r_frame, const InstanceImage& r_instance_image,
        ...
        inline std::size_t size() const { return m_label_to_objects.size(); }
       private:
