...
template<class T> class SimpleQueueSynchronizer : public QueueSynchronizerBase<T>
{
  public:
    ...
    {
        if (!queue) spdlog::error("Queue is null");
        if (!pipeline_payload) spdlog::error("pipeline_payload is null");
        static_assert(
          std::is_base_of<PipelinePayload, typename std::pointer_traits<T>::element_type>::value,
