...
namespace oslam
{
    ...
    class ThreadsafeQueueBase
    {
       public:
        ...
        void shutdown()
        {
            spdlog::debug("Shutting down queue: {}", queue_id_);
    ...
    template<typename T>
    ThreadsafeQueue<T>::ThreadsafeQueue(const std::string& queue_id) : ThreadsafeQueueBase<T>(queue_id)
    {
    }
    ...
    bool ThreadsafeQueue<T>::push(T new_value)
    {
        ...
        if (queue_size != 0)
        {
            spdlog::debug("Queue with id: {}, is getting full, size: {}", queue_id_, queue_size);
        }
    ...
    bool ThreadsafeQueue<T>::popBlockingWithTimeout(T& value, size_t duration_ms)
    {
        ...
        data_cond_.wait_for(
            lk, std::chrono::milliseconds(duration_ms), [this] { return !data_queue_.empty() || shutdown_; });
    ...
    bool ThreadsafeQueue<T>::batchPop(typename TQB::InternalQueue* output_queue)
    {
        ...
        if (!output_queue)
            spdlog::error("Output Queue is nullptr");
...
#endif /* ifndef OSLAM_THREADSAFEQUEUE_H */
