...
Tracker::Tracker(MaskedImageQueue *p_masked_image_queue, OutputQueue *p_output_queue)
  : MISO(p_output_queue, "Tracker"), m_frame_queue("InputFrameQueue"),
    mp_masked_image_queue(p_masked_image_queue), mr_global_map(GlobalMap::get_instance())
{}
...
Tracker::InputUniquePtr Tracker::get_input_packet(void)
{
    ...
    if (!is_current_maskframe) {
        ...
        p_tracker_payload =
          std::make_unique<TrackerPayload>(m_curr_timestamp, *p_input_frame, *mp_prev_masked_image);
    } else {
        ...
        //! current frame
        if (!syncQueue<MaskedImage::UniquePtr>(
              m_curr_timestamp, mp_masked_image_queue, &p_masked_image)) {
            spdlog::error("Missing masked image with requested timestamp: {}", m_curr_timestamp);
            return nullptr;
        }
        if (!p_masked_image) {
            spdlog::error(
              "Module: {} {} returned null", name_id_, mp_masked_image_queue->queue_id_);
            return nullptr;
        }
        p_tracker_payload =
          std::make_unique<TrackerPayload>(m_curr_timestamp, *p_input_frame, *p_masked_image);
...
Tracker::OutputUniquePtr Tracker::run_once(Tracker::InputUniquePtr p_input)
{
    ...
    mr_global_map.integrate_background(
      curr_color_image, curr_depth_image, mv_T_camera_2_world.back());
    ...
    cv::Mat vertex_map(
      p_vertex_map->height_, p_vertex_map->width_, CV_32FC3, p_vertex_map->data_.data());
    cv::Mat normal_map(
