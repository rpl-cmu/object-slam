...
namespace oslam
{
    struct InstanceImage
    {
            ...
            : maskb_(mask), bbox_(bbox), bbox_mask_(mask.size(), CV_8UC1, cv::Scalar(0)), label_(label), score_(score)
        {
            ...
            // clang-format off
            if ((left_top.x - 5 < mask.cols) && (left_top.x - 5 >= 0) &&
                (left_top.y - 5 < mask.rows) && (left_top.y - 5 >= 0))
                    left_top -= cv::Point2i(5, 5);
            if ((right_bottom.x + 5 < mask.cols) && (right_bottom.x + 5 >= 0) &&
                (right_bottom.y + 5 < mask.rows) && (right_bottom.y + 5 >= 0))
