...
namespace oslam
{
    ...
    class PipelineModuleBase
    {
       ...
       public:
         ...
         * @param
         */
        PipelineModuleBase(const std::string &name_id) : name_id_(name_id) { spdlog::info("Module ({}) started", name_id); }
        ...
        [[nodiscard]] inline bool isWorking() const { return is_thread_working_ || hasWork(); }
        ...
        virtual void registerOnFailureCallback(const OnFailureCallback &callback)
        {
            if (!callback)
            {
                spdlog::error("Failure Callback does not exist");
            }
       ...
       protected:
         ...
         */
        template<class T>
        bool syncQueue(const Timestamp &timestamp, ThreadsafeQueue<T> *queue, T *pipeline_payload, int max_iterations = 10)
        {
    ...
    class MIMOPipelineModule : public PipelineModule<Input, Output>
    {
       public:
        ...
        explicit MIMOPipelineModule(const std::string &name_id) : PipelineModule<Input, Output>(name_id), output_callbacks_()
        {
        }
    ...
    class SIMOPipelineModule : public MIMOPipelineModule<Input, Output>
    {
       public:
        ...
        {
            if (!input_queue_)
                spdlog::error("Input Queue for module: {} is null!", PIO::name_id_);
        }
       ...
       protected:
        ...
        typename PIO::InputUniquePtr getInputPacket() override
        {
            ...
            {
                spdlog::warn("Module: {} - Input queue: {} didn't return an output", PIO::name_id_, input_queue_->queue_id_);
                return nullptr;
            }
        }
    ...
    class SISOPipelineModule : public MISOPipelineModule<Input, Output>
    {
       public:
        ...
        SISOPipelineModule(InputQueue *input_queue, OutputQueue *output_queue, const std::string &name_id)
       ...
       protected:
        ...
        typename MISO::InputUniquePtr getInputPacket() override
        {
            ...
            {
                spdlog::warn(
                    "Module: {} - Input queue: {} didn't return an output", MISO::name_id_, input_queue_->queue_id_);
                return nullptr;
            }
        }
