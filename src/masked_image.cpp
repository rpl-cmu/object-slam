...
namespace oslam
{
  MaskedImage::MaskedImage(const Timestamp timestamp, const open3d::geometry::Image &r_image,
                           std::vector<unsigned int> labels, std::vector<double> scores)
  ...
  std::vector<cv::Mat> MaskedImage::process()
  {
    ...
    for (unsigned int i = 0; i < m_labels.size(); i++)
    {
      ...
      mat_masks.emplace_back(object_mask);
    }
