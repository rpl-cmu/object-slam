...
bool DataReader::run()
{
    if (!m_shutdown && !m_dataset_parsed) m_dataset_parsed = parse_dataset();

    if (m_dataset_parsed) {
        while (!m_shutdown && read_frame()) { ++m_current_index; }
    }

    if (m_shutdown) spdlog::info("DataReader Shutdown requested");
    return false;
}
...
bool DataReader::parse_dataset(void)
{
    ...
    spdlog::debug(
      "{}, {}, {}", color_files_path.string(), depth_files_path.string(), mask_files_path.string());
